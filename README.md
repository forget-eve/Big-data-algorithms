...
### 2.2 Chebyshev’s Inequality(切比雪夫不等式)
...
> - **Proof.** 在此处先证明 $Pr[S > (1 + δ)µ]$ 部分，并假定考虑 $X_i$ 为 `Bernoulli` 变量(非0即1)时(即 $Pr[X_i = 1] = p_i, Pr[X_i = 0] = 1 − p_i$ )( $\lambda > 0$ )
...
> - 上式中 $(3)$ 可直接由 `Markov’s Inequality` 得到(很明显，随机变量 $e^{λS}$ 和取值 $e^{λ(1+δ)µ}$ 均大于 $0$ ，有不等式的第二种形式可得到)。 $(4)$ 由 $X_i$ 独立可得 $E[e^{λS}]=E[e^{\sum\limits_{i=1}^n λX_i}]=E[\prod\limits ^n_{i=1} e^{λX_i}]=\prod\limits ^n_{i=1}E[e^{λX_i}]$。 $(5)$ 利用不等式 $1 + x ≤ e^x$ 进行放缩获得 $(5)$ 到下一步是由于 $\mu =E[S]=E[\sum\limits_{i=1}^n X_i]$ ，由于 $X_i$ 相互独立，所以 $\mu =\sum\limits_{i=1}^n E[X_i]=\sum\limits_{i=1}^n p_i$ ，所以 $\prod\limits ^n_{i=1} e^{p_i(e^λ−1)}=e^{\sum\limits_{i=1}^n p_i(e^{\lambda}-1)}$。令 $e^λ = 1 + δ$ 代入得到  $(6)$。对上式最后的结果取对数分析：
...
> - 最后由于 $0 ≤ X_i ≤ 1$ ，所以不难有 $S=\sum\limits^n_{i=1} X_i \leq n$ 。那么 $\mu =E[S] \leq n$ $\frac{\mu}{n} = \frac{E[S]}{n} \leq 1$ 。此时看上式中 $\frac{δ^2}{3}µ \geq \frac{δ^2}{3}µ · \frac{\mu}{n}=\frac{δ^2µ^2}{3n} \geq \frac{δ^2µ^2}{n}$ 。所以 $\frac{δ^2}{3}µ=O(\frac{δ^2µ^2}{n})$

> - 关于 `Chernoff Bound` 的证明还可以使用其他的放缩方式，需要使用下述引理：

- [x] **Lemma 2.4 ( `Hoeffding lemma` (霍夫丁引理))**. 假设随机变量 $a ≤ X ≤ b$ 且 $E[X] = 0$ ，那么对于 $∀λ$，有

$$E[e^{λX}] ≤ e^{\frac{λ^2(b − a)^2}{8}}$$

> 关于 `Hoeffding lemma` 的证明 <kbd><a href="https://www.cnblogs.com/vmkash/p/13852391.html">点击此处</a></kbd>
...
> - 上式最后一步中，令 $λ =\frac{4δµ}{n}$  即可得到对应结果。 由于 $2$ 是常数可以忽略所以 $\frac{2δ^2µ^2}{n}=O(\frac{δ^2µ^2}{n})$

> - 现在再来证明 $Pr[S < (1 - δ)µ]$ 的情况，证明过程和上面大同小异。

$$Pr[S < (1 - δ)µ]=Pr[-S > -(1 - δ)µ]=Pr[e^{-λS} > e^{-λ(1-δ)µ}]≤\frac{1}{e^{-λ(1-δ)µ}}E[e^{-λS}]=e^{λ(1-δ)µ}E[e^{-λS}]$$

$$=e^{λ(1-δ)µ}\prod\limits ^n_{i=1} E[e^{-λX_i}]=e^{λ(1-δ)µ}\prod\limits ^n_{i=1} (p_ie^{-λ} + (1 − p_i)) ≤e^{λ(1-δ)µ} \prod\limits ^n_{i=1} e^{p_i(e^{-λ}-1)} = e^{λ(1-δ)µ}·e^{µ(e^{-λ}-1)} = e^{λ(1-δ)µ+µ(e^{-λ}-1)}=(e^{λ(1-δ)+(e^{-λ}-1)})^{\mu}$$

> - 这里令 $e^{-λ}-1=-δ$ ，此处和上面情况不一样是因为 $0< e^{-λ} <1$ , 而 $e^{λ}>1$ 。

$$=(e^{λ(1-δ)}·e^{e^{-λ}-1})^µ=((\frac{1}{e^{-λ}})^(1-δ)·e^{-δ})^µ=(\frac{e^{-δ}}{(1-δ)^{1-δ}})^µ$$

> - 对最后的结果取对数那么有

$$\ln (\frac{e^{-δ}}{(1-δ)^{1-δ}})^µ =µ(-δ-(1-δ)\ln (1-δ)) \leq -µδ^2$$

> - 由泰勒展开，不难有不等式 $\ln (1-x) \geq -x$ ，所以有最后一个不等式。跟上面一致可证， $µδ^2=O(\frac{δ^2µ^2}{n})$

> - 第二种方法也是引入 `Hoeffding` 引理，那么同前面有

$$Pr[S < (1 - δ)µ]≤e^{λ(1-δ)µ}E[e^{-λS}]$$

> - 那么此时只用求出 $E[e^{-λS}]$ 的界限，则同前有

$$E[e^{-λS}]=\prod\limits ^n_{i=1} E[e^{-λX_i}]=\prod\limits ^n_{i=1} e^{-λp_i}·E[e^{-(x_i-p_i)}] ≤ =\prod\limits ^n_{i=1} e^{-λp_i}·e^{\frac{λ^2}{8}}=e^{-λµ+\frac{λ^2}{8}}$$

> - 上面的倒数第二步不等式放缩不是 $e^{-\frac{λ^2}{8}}$ 是因为 `Hoeffding` 引理对 $λ$ 取值是任意的，正负均可，那么简单来说，这一步可以将 $-λ$ 看成整体带入平方，所以不等式变换中就没有负号。

$$Pr[S < (1 - δ)µ]≤e^{λ(1-δ)µ}E[e^{-λS}] ≤e^{λ(1-δ)µ} · e^{-λµ+\frac{λ^2}{8}} =e^{\frac{λ^2}{8}n−λδµ}=e^{−\frac{2δ^2µ^2}{n}}$$

> - 最后一步也是同前一种情况中令 $λ =\frac{4δµ}{n}$  即可得到对应结果。

> - 那么可以得到 $Pr[|S − µ| ≤ δµ]=1-Pr[S < (1 - δ)µ]-Pr[S > (1 + δ)µ] ≥ 1 − 2e^{−O(\frac{δ^2µ^2}{n})} ≥ 1 − e^{−O(\frac{δ^2µ^2}{n})+ln2} ≥ 1 − e^{−O(\frac{δ^2µ^2}{n})}$
...
> - `Chernoff Bound` 可以扩展到任何 `次高斯` (sub-gaussian)随机变量。

- [x] **Definition 1.1.** 随机变量被称为次高斯随机变量，如果存在 $c > 0, σ$ , 使得对充分大的 $t$ , 有

$$P(x ≥ t) ≤ ce^{−\frac{t^2}{2σ^2}}.$$

> - 关于 [`次高斯` (sub-gaussian)随机变量的性质](https://blog.csdn.net/weixin_43413559/article/details/121492414)。

- [x] **Example 1.2.** (次高斯随机变量是)支撑集<sup id="return1"><a href="#/?id=footnote1">➹</sup>为闭区间(或其他 $\mathbb{R}^d$ 上 [紧集](https://zhuanlan.zhihu.com/p/403213165)) 的连续分布. 原因是在一个区间内有界而他处为 $0$ .
  > - 这句话的含义是指次高斯随机变量的支撑集通常是一个闭区间，或者更一般地说，是 $\mathbb{R}^d$ 上的一个紧集。这是因为次高斯分布在一个有限的区间内有非零的概率密度，而在其他地方的概率密度为零。
  > - 具体来说，次高斯随机变量是指具有截尾分布的随机变量，即其概率密度函数在某个有限的区间内非零，而在其他地方为零。因此，次高斯随机变量的支撑集通常是与其概率密度函数的非零部分对应的那个区间或紧集。
  > - ~~举个例子，考虑一个以均值为 $\mu$、方差为 $\sigma^2$ 的正态分布。如果我们将这个正态分布在某个有限的区间 $[a, b]$ 上截断，那么截断后的分布就成为一个次高斯随机变量。在这种情况下，支撑集就是闭区间 $[a, b]$，因为在该区间内有非零的概率密度，而在其他地方的概率密度为零。~~

> - 注：上面 `紧集的链接` 仅看定义，例子有问题。更好理解的方式： ***在欧式空间中，紧集等价于有界闭集*** ，例如，在实数轴上，闭区间 $[a, b]$ 是一个紧集，因为它既是有界的又是闭的。
...
- 注： `Hoeffding Bound` 和 `Chernoff Bound` 的区别：
  > - `Hoeffding Bound` ：
  > > - 适用于有界独立同分布的随机变量，例如服从二项分布或指数分布的随机变量。
  > > - 提供了对样本均值的偏离进行上界估计，通常是以概率形式给出。
  > > - 通常用于分析随机过程中的收敛性，也常用于分析集中性(concentration)问题，例如估计样本均值与总体均值之间的差异。例如在机器学习中的随机梯度下降算法的收敛速度分析中常用。
  >
  > - `Chernoff Bound` ：
  > > - 更一般化，适用于独立同分布或不独立的随机变量序列，且不一定需要有界。
  > > - 通常给出随机变量与其期望之间的差异的概率界，通常以指数形式给出。
  > > - 提供了更紧密的界限，但需要更多的假设和计算，常用于分析概率估计的精度，例如在概率近似算法或机器学习中用于分析算法的性能。

- [x]  **Example 1.5.** 若 $X_i \in [a_i, b_i]$ , 则有

$$P \left(|\sum\limits_{i=1}^n (x_i − EX_i)| ≥ t \right) ≤ 2e^{− \frac{2t^2}{\sum\limits_{i=1}^n (b_i−a_i)^2}}$$

- [x] **Remark 1.6.** 事实上, 上式对任意随机变量都成立, 并不局限于次高斯变量.
...
> 则称之为一个鞅(martingale)(/ `下鞅` (submartingale),又称 `亚鞅` / `上鞅` (supermartingale)，又称 `超鞅` )序列.( <kbd><a href="https://zh.wikipedia.org/wiki/%E9%9E%85_(%E6%A6%82%E7%8E%87%E8%AE%BA)">详细定义建议看此处</a></kbd>)

- [x] **Example 1.8.** 一个人参加一系列赌博游戏, 每次游戏带来的收益均值都为 $0$ . 则此人的本金(构成的序列) 是一个鞅.
  > - 关于上、下鞅的的直观解释:
  > - 上鞅表示第 $n+1$ 年的平均赌本 `不多于` 第 $n$ 年的赌本，即具有 `上鞅` 这种性质的赌博是 `亏本` 赌博;
  > - 下鞅表示第 $n+1$ 年的平均赌本 `不少于` 第 $n$ 年的赌本，即具有 `下鞅` 这种性质的赌博是 `盈利` 赌博。

- [x] **Theorem 1.9 ( `Azuma` 不等式).**  对于上述的鞅/下鞅, 如满足 $∀i, z_i −z_{i−1} ∈ [a_i, b_i], |a_i −b_i| ≤ c_i$ ,则

$$P (z_j − z_0 ≤ −t) ≤ e^{− \frac{t^2}{\sum\limits_{i=1}^{j} c_i^2}}$$

> - **Proof.**

$$P (z_j − z_0 ≤ −t) = P \left( \sum\limits_{i=1}^j (z_{i-1}-z_i) \geq t \right)$$

$$\ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ ≤ e^{-λt}E[e^{λ \sum\limits_{i=1}^j (z_{i-1}-z_i)}] \ \ \ \ (Chernoff Bound的推导过程)$$
 
$$\ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ = e^{−λt}E\left[ E[e^{λ\sum\limits_{i=1}^j (z_{i−1}−z_i)}|z_{j−1},\dots , z_1] \right] \ \ \ \  (全期望公式)$$

$$\ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ = e^{−λt}E\left[ e^{λ\sum\limits_{i=1}^{j-1} (z_{i−1}−z_i)} E[e^{λz_{j−1}−z_j)}|z_{j−1},\dots , z_1] \right]$$

$$\ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ = e^{−λt}E\left[ e^{λ\sum\limits_{i=1}^{j-1} (z_{i−1}−z_i)} E[e^{λE[z_j]−z_j)}|z_{j−1},\dots , z_1] \right] \ \ \ \ (鞅的定义)$$

$$\ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ ≤ e^{−λt}E\left[ e^{λ\sum\limits_{i=1}^{j-1} (z_{i−1}−z_i)} e^{-λ^2 \frac{(b_j-a_j)^2}{8}}|z_{j−1},\dots , z_1 \right] \ \ \ \ (Hoeffding 引理)$$

$$\ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ ≤ e^{−λt}e^{- \frac{λ^2\sum\limits_{i=1}^j(b_i-a_i)^2}{8}} \ \ \ \ (递推)$$

$$\ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ ≤ e^{− \frac{t^2}{\sum\limits_{i=1}^{j} c_i^2}} \ \ \ \ (优化λ)$$

> 全期望公式： $E[E[X|Y]]=E[X]$
> > 证明：$E[X]=\sum\limits_{y_i \in \mathbb{Y}} E[X|Y=y_i]P(Y=y_i)=E[E[X|Y]]$

> - **Remark 1.10.** 同理可证 $P (z_j − z_0 \geq −t) ≤ e^{− \frac{t^2}{\sum\limits_{i=1}^{j} c_i^2}}$

## 4 Chaining

- [x] 上述 `集中不等式` 提供了随机变量均值的尾概率, 要研究一族随机变量的上界 (同样是随机变量) 的尾概率, 引入 `Chaining` 方法. 以下研究一个具体的案例, 从中管窥 `chaining` 方法的一角。
...
<div id="footnote" style="font-size:10px">
  <h6 id="footnote1">
支撑集(support set):是指随机变量取值的集合中，在概率密度函数或概率质量函数非零的部分。换句话说，支撑集是随机变量可能取值的集合中实际出现的部分。<a href="#/?id=return1">☁返回</a></h6>
  </h6>
</div>
